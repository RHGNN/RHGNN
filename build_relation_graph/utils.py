...
@jit(nopython=True)
def random_walk(edges, node_v, node_ixs, rwlen, p=1, q=1, n_walks=1):
    N = len(node_ixs)
    ...
    for w in range(n_walks):
        ...
        # source_node = node_ixs[source_node]
...
class RandomWalker:
    ...
    def walk(self):
        ...
        while True:
            yield random_walk(self.edges, self.node_v, self.node_ixs, self.rw_len, self.p, self.q,
...
def edge_path_from_random_walk(graph, rws):
    ...
    for rw in rws:
        ...
        test_rlen = len(row_index.eq(1))

        col_index = col.eq(node_des)

        test_clen = len(col_index.eq(1))

        common_index = (row_index + col_index).eq(2)

        max = torch.max(common_index)
